...
function HallLayer:onCreate(parames)
    ...
    local uiImage_BT = ccui.Helper:seekWidgetByName(self.root,"Image_Bitch")
    self.boss = 0
    Common:addTouchEventListener(uiImage_BT,function() 
        ...
        uiImage_BT:setEnabled(false)
        self:runAction(cc.Sequence:create(cc.DelayTime:create(3),cc.CallFunc:create(function(sender,event)  uiImage_BT:setEnabled(true)  end)))
    end,true)
    --uiImage_BT   
    if self.boss == 0 then 
        self:runAction(cc.Sequence:create(cc.DelayTime:create(0),
            cc.CallFunc:create(function(sender,event) 
            uiImage_BT:setEnabled(false)
            self.boss = 1
            require("common.Common"):playEffect("hall/boss1.mp3") end),
            cc.DelayTime:create(2),
            cc.CallFunc:create(function(sender,event) 
                uiImage_BT:setEnabled(false)
                uiImage_BT:setEnabled(true) end)))    
    end  
    --邀请有礼
 ...
 --   end 
    ...
    local uiText_donghua = ccui.Helper:seekWidgetByName(self.root,"Text_donghua")
    if uiText_donghua~= nil then  
        local function TextAction(sender,event)
            uiText_donghua:runAction(cc.Sequence:create(--cc.ScaleTo:create(0.2,1.0),
            cc.ScaleTo:create(0.4,1.1),cc.ScaleTo:create(0.2,1),
            -- cc.ScaleTo:create(0.4,1.1),cc.ScaleTo:create(0.2,1),
            -- cc.ScaleTo:create(0.4,1.1),cc.ScaleTo:create(0.2,1),
            -- cc.ScaleTo:create(0.1,0),
            cc.Sequence:create(cc.DelayTime:create(3),cc.CallFunc:create(TextAction))))           
        end      
        TextAction()
    end
