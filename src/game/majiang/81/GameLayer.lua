...
function GameLayer:readBuffer(luaFunc, mainCmdID, subCmdID)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        ...
        elseif subCmdID == NetMsgId.SUB_S_OPERATE_RESULT then              --操作结果
            ...
            for i = 1 , 4 do
                _tagMsg.pBuffer.lGameScore[i] = luaFunc:readRecvLong()
                print("分数",i,_tagMsg.pBuffer.lGameScore[i])
            end
            _tagMsg.pBuffer.cbPublicCard = luaFunc:readRecvByte()       --用户扑克
            -- 湘西猪笼子独有(暂时)
            _tagMsg.pBuffer.cbOperateType = luaFunc:readRecvByte()      --杠的类型     杠的类型  1.公杠  2.暗杠  3.明杠
            _tagMsg.pBuffer.bIsGangData = luaFunc:readRecvByte()        --1:有杠分数据  0.无杠分数据
            print("杠的类型+++++++++++++++++",_tagMsg.pBuffer.wOperateUser,_tagMsg.pBuffer.wProvideUser,_tagMsg.pBuffer.cbOperateCode,_tagMsg.pBuffer.cbOperateCard,_tagMsg.pBuffer.cbUserCardCout,_tagMsg.pBuffer.cbPublicCard,_tagMsg.pBuffer.cbOperateType,_tagMsg.pBuffer.bIsGangData)
...
function GameLayer:OnGameMessageRun(_tagMsg)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        ...
        elseif subCmdID == NetMsgId.SUB_S_OPERATE_RESULT then              --操作结果
            GameCommon.iNOoutcard = false
            --杠分处理
            if Bit:_and(pBuffer.cbOperateCode,GameCommon.WIK_GANG) ~= 0 and pBuffer.bIsGangData == 1 then --Bit:_and(pBuffer.cbOperateCode,GameCommon.WIK_GANG) ~= 0 and
                -- for i = 1, GameCommon.gameConfig.bPlayerCount do
                --     local wChairID = i-1
                --     GameCommon.player[wChairID].gangFen = GameCommon.player[wChairID].gangFen + pBuffer.lGameScore[i]
                -- end

                -- self.tableLayer:showHUScore(pBuffer)
        ...
        elseif subCmdID == NetMsgId.SUB_S_GAME_END_MAJIANG then                    --游戏结束
            ...
            cc.CallFunc:create(function(sender,event)          
                uiPanel_end:addChild(require("game.majiang.81.GameEndLayer"):create(pBuffer))                         
            end),
