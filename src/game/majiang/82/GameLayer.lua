...
function GameLayer:readBuffer(luaFunc, mainCmdID, subCmdID)
    ...
    if mainCmdID == NetMsgId.MDM_GR_USER then   
       if subCmdID == NetMsgId.SUB_GR_USER_READY then
        ...
        elseif subCmdID == NetMsgId.SUB_GR_USER_COME then 
            ...
            data.buFen = 0 --data.lScore --做补分临时显示
            data.cardNode = {}
            printInfo(data)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        ...
        elseif subCmdID == NetMsgId.SUB_S_Start_HU then             --通知客户端必胡操作
            _tagMsg.pBuffer.wCurUser = luaFunc:readRecvWORD()       --玩家
        ...
        elseif subCmdID == NetMsgId.SUB_S_OPERATE_NOTIFY_MAJIANG then              --操作提示
            ...
            _tagMsg.pBuffer.cbGangCard = {}                             -- 杠牌
            for i = 1, 4 do
                _tagMsg.pBuffer.cbGangCard[i] = luaFunc:readRecvByte()
            end
            _tagMsg.pBuffer.cbBuCard = {}                               --补牌
            for i = 1, 4 do
        ...
        elseif subCmdID == NetMsgId.SUB_S_GAME_END_MAJIANG then                    --游戏结束
            ...
            _tagMsg.pBuffer.mYMQZNiaoCOUNT = {}                         --鸟分 
            for i = 1, 4 do
                _tagMsg.pBuffer.mYMQZNiaoCOUNT[i] = luaFunc:readRecvByte()
            end

            _tagMsg.pBuffer.mFanCount = {}                         --番数 
            for i = 1, 4 do
                _tagMsg.pBuffer.mFanCount[i] = luaFunc:readRecvByte()
            end
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_FRAME then
        if subCmdID == NetMsgId.SUB_GF_SCENE then 
            ...
            _tagMsg.pBuffer.bStartHu = luaFunc:readRecvByte()  -- 起手胡牌断线重连判定条件
...
function GameLayer:OnGameMessageRun(_tagMsg)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        ...
        elseif subCmdID == NetMsgId.SUB_S_SEND_CARD_MAJIANG then                   --发牌消息
            ...
            self:updatehandplate()
        elseif subCmdID == NetMsgId.SUB_S_Start_HU then                   --通知客户端必胡操作
            self.tableLayer:doAction(NetMsgId.SUB_S_Start_HU, pBuffer)
        elseif subCmdID == NetMsgId.SUB_S_JIA_PIAO then
            ---加票
            self.tableLayer:updateGameState(GameCommon.GameState_Start)
        ...
        elseif subCmdID == NetMsgId.SUB_S_OPERATE_RESULT then              --操作结果
            ...
            --补分处理
            if Bit:_and(pBuffer.cbOperateCode,GameCommon.WIK_FILL) ~= 0 then
                for i = 1, GameCommon.gameConfig.bPlayerCount do
                    local wChairID = i-1
                    GameCommon.player[wChairID].gangFen = GameCommon.player[wChairID].gangFen + pBuffer.lGameScore[i]
                end
        ...
        elseif subCmdID == NetMsgId.SUB_S_GAME_END_MAJIANG then                    --游戏结束
            ...
            cc.CallFunc:create(function(sender,event)          
                uiPanel_end:addChild(require("game.majiang.82.GameEndLayer"):create(pBuffer))                         
            end),
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_FRAME then
        if subCmdID == NetMsgId.SUB_GF_SCENE then
            ...
            for i = 1, GameCommon.gameConfig.bPlayerCount do
                ...
                self.tableLayer:showHandCard({wChairID = i-1}) --wOutCardUser
            end
            ...
            if pBuffer.cbActionMask ~= 0 then
                if pBuffer.bStartHu ~= 0 then 
                    pBuffer.wCurUser = GameCommon:getRoleChairID() 
                    print("断线重连++++起手胡牌",pBuffer.wCurUser)
                    self.tableLayer:doAction(NetMsgId.SUB_S_Start_HU, pBuffer)
                else
                    if pBuffer.wOutCardUser == GameCommon:getRoleChairID() or pBuffer.wOutCardUser == 65535 then
                        local a =  Bit:_and(pBuffer.cbActionMask,GameCommon.WIK_CHI_HU_EX)
                        self.tableLayer:doAction(NetMsgId.SUB_S_OPERATE_NOTIFY_MAJIANG,{wResumeUser = 0,cbActionMask = pBuffer.cbActionMask, 
                            cbActionCard = pBuffer.cbActionCard,bIsSelf = true,cbGangCard = pBuffer.cbGangCard,cbBuCard = pBuffer.cbBuCard, wLastOutCardUser = pBuffer.wLastOutCardUser})
                    else
                        self.tableLayer:doAction(NetMsgId.SUB_S_OPERATE_NOTIFY_MAJIANG,{wResumeUser = 0,cbActionMask = pBuffer.cbActionMask, 
                            cbActionCard = pBuffer.cbActionCard,bIsSelf = false,cbGangCard = pBuffer.cbGangCard,cbBuCard = pBuffer.cbBuCard,wLastOutCardUser = pBuffer.wLastOutCardUser})
                    end
                end
            end
