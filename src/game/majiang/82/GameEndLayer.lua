...
function GameEndLayer:onCreate(pBuffer)   
    ...
    uiText_Gamename:setString("湘西红中麻将") 
    ...
    for i = 1,GameCommon.gameConfig.bPlayerCount do
        ...
        local desc = ""
        print("胡牌类型+++++++++++++++++++++++++++++",i,pBuffer.wChiHuKind[i])
        desc = desc.. self:showMingTang(pBuffer.wChiHuKind[i])--,pBuffer.mFanCount[i]

        if pBuffer.mYMQZNiaoCOUNT[i] ~= 0  then--and pBuffer.wWinner[i] == true
...
-- #define CHK_SIXI_HU					0x0002								//四喜胡牌
-- #define CHK_BANBAN_HU				0x0004								//板板胡牌
-- #define CHK_LIULIU_HU				0x0008								//六六顺牌
-- #define CHK_QUEYISE_HU				0x0010								//缺一色牌
  ...
  --  local CHR_QUAN_QIU_REN =			0x0010								--全求人
    ...
    local CHR_GANG_PAO	=			0x0200								--杠上炮			--权位
    ...
    --杠上炮
    if Bit:_and(wChiHuKind,CHR_GANG_PAO) ~= 0 then
        desc =desc.." 杠上炮"      
    end 
    --抢杠胡
