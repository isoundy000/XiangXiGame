...
local TableLayer = require("game.paohuzi.88.TableLayer")
...
function GameLayer:startGame(...)
    ...
    local csb = cc.CSLoader:createNode("GameLayerErQiShiPai.csb")
    self:addChild(csb)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        if subCmdID == NetMsgId.RET_SC_GAME_CONFIG then
            ...
            if GameCommon.gameConfig.bPlayerCount <= 2 then
                local uiPanel_SendCard3 = ccui.Helper:seekWidgetByName(self.root,"Panel_SendCard3")
                uiPanel_SendCard3:setVisible(false)
                local uiPanel_discardCard3 = ccui.Helper:seekWidgetByName(self.root,"Panel_discardCard3")
                uiPanel_discardCard3:setVisible(false)
                local uiText_discardCard3 = ccui.Helper:seekWidgetByName(self.root,"Text_discardCard3")
                uiText_discardCard3:setVisible(false)
                local uiText_outCard3 = ccui.Helper:seekWidgetByName(self.root,"Text_outCard3")
                uiText_outCard3:setVisible(false)
            end
        ...
        elseif subCmdID == NetMsgId.SUB_S_SELECT_PIAOFEN then
            self.tableLayer:updateGameState(GameCommon.GameState_Start)
            local data = {}
            data.bSelectPiaoFen = luaFunc:readRecvBool()
            data.bSelectPiaoFenFinish = {}
            for i=0,3 do
                data.bSelectPiaoFenFinish[i] = luaFunc:readRecvBool()
            end
            data.cbPiaoFen = {} 
            for i=0,3 do
                data.cbPiaoFen[i] = luaFunc:readRecvByte()
            end
            self:updatePlayerPiaoFen(data)
            
             local uiPanel_action = ccui.Helper:seekWidgetByName(self.root,"Panel_action")
            local node = uiPanel_action:getChildByName("SelectPiaoFenLayer")
            if node then
                node:removeFromParent()
            end
            if data.bSelectPiaoFen == true and data.bSelectPiaoFenFinish[GameCommon:getRoleChairID()] == false then
                local visibleSize = cc.Director:getInstance():getVisibleSize()
                local csb = cc.CSLoader:createNode("SelectPiaoFenLayer.csb")
                uiPanel_action:addChild(csb)
                csb:setName("SelectPiaoFenLayer")
                local root = csb:getChildByName("Panel_root")
                local uiButton_item = ccui.Helper:seekWidgetByName(root, "Button_item")
                uiButton_item:retain()
                root:removeAllChildren()
                local item = uiButton_item:clone()
                item:loadTextures("game/ok_ui_s_btn_blue.png","game/ok_ui_s_btn_blue.png","game/ok_ui_s_btn_blue.png")
                item:setTitleText("不充分")
                item.cbPiaoFen = 0
                root:addChild(item)
                local tablePiaoFen = {2,5,10,15}
                for k,v in pairs(tablePiaoFen) do
                    local item = uiButton_item:clone()
                    item:setTitleText(string.format("充%d分",v))
                    item.cbPiaoFen = v
                    root:addChild(item)
                end
                local items = root:getChildren()
                local width = uiButton_item:getContentSize().width*#items+30*(#items-1)
                local beginPos = visibleSize.width/2-width/2+uiButton_item:getContentSize().width/2
                local step = uiButton_item:getContentSize().width + 30
                for k,v in pairs(items) do
                    v:setPosition(beginPos + (k-1)*step ,visibleSize.height*0.4)
                    Common:addTouchEventListener(v,function()
                        NetMgr:getGameInstance():sendMsgToSvr(NetMsgId.MDM_GF_GAME,NetMsgId.SUB_C_SELECT_PIAOFEN,"b",v.cbPiaoFen)
...
function GameLayer:OnGameMessageRun(_tagMsg)
    ...
    elseif mainCmdID == NetMsgId.MDM_GF_GAME then
        if subCmdID == NetMsgId.SUB_S_GAME_START then
            if GameCommon.gameConfig.bChongFen ~= 1 then
                self.tableLayer:updateGameState(GameCommon.GameState_Start)
            end
            --开始游戏
            ...
            for i = 0, 3 do
                if GameCommon.player[i] ~= nil then
                    if i == pBuffer.wBankerUser then
                         GameCommon.player[i].bUserCardCount = 17
                    else
                        GameCommon.player[i].bUserCardCount = 16
                    end
                end
            ...
            cbLeftCardCount = cbLeftCardCount - 16*GameCommon.gameConfig.bPlayerCount - 1
            ...
            if pBuffer.wWinUser ~= GameCommon.INVALID_CHAIR then
                uiPanel_end:runAction(cc.Sequence:create(
                    ...
                    cc.CallFunc:create(function(sender,event) 
                        uiPanel_end:addChild(require("game.paohuzi.88.GameEndLayer"):create(pBuffer))
                    end),
...
function GameLayer:updatePlayerPiaoFen(data)
    ...
    for wChairID=0,GameCommon.gameConfig.bPlayerCount-1 do
        local viewID = GameCommon:getViewIDByChairID(wChairID)
        local uiPanel_player = ccui.Helper:seekWidgetByName(self.root,string.format("Panel_player%d",viewID))
        local uiText_huXi = ccui.Helper:seekWidgetByName(uiPanel_player,"Text_huXi")
        if data.bSelectPiaoFenFinish[wChairID] == false then
            uiText_huXi:setString("等待充分")
        elseif data.cbPiaoFen[wChairID] == 0 then
            uiText_huXi:setString("不充分")
        else
            uiText_huXi:setString(string.format("充%d分",data.cbPiaoFen[wChairID]))
        end
