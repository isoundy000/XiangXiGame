...
local GameOperation = require("game.paohuzi.88.GameOperation")
...
function TableLayer:setDiscardCard(wChairID, bDiscardCardCount, bDiscardCard,bOutCardMark)
    ...
    local uiPanel_SendCard = ccui.Helper:seekWidgetByName(self.root,string.format("Panel_SendCard%d",viewID))
    -- if isThreeRoom then
    --     local parentWidth = uiPanel_discardCard:getParent():getContentSize().width
    --     uiPanel_discardCard:setPositionX(parentWidth * 0.88) 
    --     uiPanel_SendCard:setPositionX(parentWidth * 0.88) 
    -- end
    uiPanel_discardCard:removeAllChildren()
    uiPanel_SendCard:removeAllChildren()
    ...
    for i = 1, bDiscardCardCount do
		...
		if isMask then
            ...
            local anchorPoint = uiPanel_discardCard:getAnchorPoint()
             local stepX = cardWidth
            local stepY = cardHeight
            local beganX = cardWidth/2 + 10
            if anchorPoint.x == 1 then
                beganX = size.width - cardWidth/2 - 10
                stepX = -cardWidth
            end
            local beganY = cardHeight/2 + 10
            if anchorPoint.y == 1 then
                beganY = size.height - cardHeight/2 - 10
                stepY = -cardHeight
            end
            
            local index = math.floor(outIndex/maxRow)
            local row = outIndex%maxRow
            outIndex = outIndex + 1
            card:setPosition(beganX + stepX*row-1 ,beganY + stepY*index)
        else
            uiPanel_SendCard:addChild(card)

            local anchorPoint = uiPanel_SendCard:getAnchorPoint()
            local stepX = cardWidth
            local stepY = cardHeight
            local beganX = cardWidth/2 + 10
            if anchorPoint.x == 1 then
            ...
            local index = math.floor(sendIndex/maxRow)
            local row = sendIndex%maxRow
                    ...
                    elseif event == ccui.TouchEventType.moved then
                        ...
                        if self.copyHandCard ~= nil then
                            ...
                            elseif row > nCardStackCount then   --插入最右边
                                ...
                                if(GameCommon.player[wChairID].cardStackInfo[row].nCardCount < 4) then
                                    card:removeFromParent()
                                    v.node = nil
                                    local _cardData = {}
                                    _cardData.data = v.data
