...
function GameEndLayer:showMingTang(pBuffer)
    ...
    local PHZ_HT_ZIMO = 1                --自　摸   自己摸出的牌胡了    加1囤
    local PHZ_HT_HONGHU = 2              --红　胡   ①红字=4只；②红字=7只；③红字>10只 and <13只                                           (X 2倍)
    local PHZ_HT_ZHENGDIANHU = 4         --点胡    只有一张红字                                                                           (X 3倍)
    local PHZ_HT_HONGWU = 16             --红乌    红字＞＝13                                                                             (X 4倍)
    local PHZ_HT_HEIWU = 32              --黑乌    红字＝0，即全部是黑字                                                                   (X 4倍)
    local PHZ_HT_DUIDUIHU = 64           --对对胡   全部对子                                                (X 4倍)                                                                                      (X 4倍)
    local PHZ_HT_HONGDUI = 128           --红对(6)                                             (X 6倍)
    local PHZ_HT_WUDUI = 256             --乌对(8)                                             (X 8倍)
    local PHZ_HT_JIAHONGDUI = 512        --夹红对(8)                                            (X 8倍)
    local PHZ_HT_TIANHU = 1024           --          

    if Bit:_and(pBuffer.wType,PHZ_HT_HONGHU)~= 0 then
    ...
    if Bit:_and(pBuffer.wType,PHZ_HT_HONGDUI)~= 0 then
        local item = uiPanel_defaultPalyer:clone()
        local uiText_mingTang = ccui.Helper:seekWidgetByName(item,"Text_mingTang")
        uiText_mingTang:setTextColor(cc.c3b(255,165,0))
        local uiText_mingTangNumber = ccui.Helper:seekWidgetByName(item,"Text_mingTangNumber")
    ...
    if Bit:_and(pBuffer.wType,PHZ_HT_JIAHONGDUI)~= 0 then
        local item = uiPanel_defaultPalyer:clone()
        local uiText_mingTang = ccui.Helper:seekWidgetByName(item,"Text_mingTang")
        uiText_mingTang:setTextColor(cc.c3b(255,165,0))
        local uiText_mingTangNumber = ccui.Helper:seekWidgetByName(item,"Text_mingTangNumber")
        uiText_mingTangNumber:setTextColor(cc.c3b(255,165,0))
        uiText_mingTang:setString("夹红对")
        uiText_mingTangNumber:setString("+8番")
        uiListView_player:pushBackCustomItem(item)
    end 
    if Bit:_and(pBuffer.wType,PHZ_HT_HONGWU)~= 0 then
        local item = uiPanel_defaultPalyer:clone()
        local uiText_mingTang = ccui.Helper:seekWidgetByName(item,"Text_mingTang")
        uiText_mingTang:setTextColor(cc.c3b(255,165,0))
        local uiText_mingTangNumber = ccui.Helper:seekWidgetByName(item,"Text_mingTangNumber")
    ...
    if Bit:_and(pBuffer.wType,PHZ_HT_ZHENGDIANHU)~= 0 then
        local item = uiPanel_defaultPalyer:clone()
        local uiText_mingTang = ccui.Helper:seekWidgetByName(item,"Text_mingTang")
        uiText_mingTang:setTextColor(cc.c3b(255,165,0))
        local uiText_mingTangNumber = ccui.Helper:seekWidgetByName(item,"Text_mingTangNumber")
    ...
    if Bit:_and(pBuffer.wType,PHZ_HT_WUDUI)~= 0 then
        local item = uiPanel_defaultPalyer:clone()
        local uiText_mingTang = ccui.Helper:seekWidgetByName(item,"Text_mingTang")
        uiText_mingTang:setTextColor(cc.c3b(255,165,0))
        local uiText_mingTangNumber = ccui.Helper:seekWidgetByName(item,"Text_mingTangNumber")
