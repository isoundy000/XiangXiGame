...
local NiuPersonInfor	= class("NiuPersonInfor", cc.load("mvc").ViewBase)

function NiuPersonInfor:onConfig()
	self.widget			= {
		{'mask','onClose'},
		{'Image_avatar_player'},
		{'Text_name'},
...
function NiuPersonInfor:onEnter()
	
end

function NiuPersonInfor:onExit()
	cc.UserDefault:getInstance():setBoolForKey('PDKOpenUserEffect', self.isOpen)
end

function NiuPersonInfor:onCreate(param)
	local data = param[1]
...
function NiuPersonInfor:updateInfo(data)
...
function NiuPersonInfor:updateControl( ... )
...
function NiuPersonInfor:onControl()
    --是否开启
    self.isOpen = not self.isOpen
    self:updateControl()
end

function NiuPersonInfor:getPlayerInfoByUserID(dwUserID)
...
function NiuPersonInfor:updateScrollviewImage()	
...
function NiuPersonInfor:buttonCall(sender)
...
function NiuPersonInfor:addListener(btn, callback)
...
function NiuPersonInfor:onClose()
	self:removeFromParent()
end
return NiuPersonInfor 