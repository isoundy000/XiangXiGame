...
local WSKGameCommon = 
{
    ...
    --0.取消  1.单牌 2.对子 3.三张 4.三带一 5.三带二  6.四带一 7.顺子  8.连对  9.三顺 10.炸弹   
    --BYTE							wOutCardType;						//出牌类型
    
    MAX_COUNT = 15,
    CardType_error          = 0,    --错误牌型  取消
    CardType_single         = 1,    --单牌
    CardType_pair           = 2,    --对子
    CardType_three          = 3,    --三条类型    
    CardType_3Add1          = 4,    --三带一
    CardType_3Add2          = 5,    --三带二
    CardType_4Add1          = 6,    --四带一
    CardType_straight       = 7,    --顺子
    CardType_straightPair   = 8,    --连对    
    CardType_airplane       = 9,    --飞机
...
function WSKGameCommon:getViewIDByChairID(wChairID)
...
function WSKGameCommon:getRoleChairID()
    return self.meChairID
end

function WSKGameCommon:ContinueGame(cbLevel)
    NetMgr:getGameInstance():sendMsgToSvr(NetMsgId.MDM_GR_USER,NetMsgId.REQ_GR_USER_SET_POSITION,"aad",LocationSystem.pos.x, LocationSystem.pos.y, WSKGameCommon.dwUserID)
...
function WSKGameCommon:getUserInfoByUserID(dwUserID)
    for key, var in pairs(self.player) do
...
function WSKGameCommon:rad(d)
    return d* math.pi / 180.0;
end 

function WSKGameCommon:GetDistance(lat1,lat2)
...
--牌资源
function WSKGameCommon:getCardNode(data)
...
function WSKGameCommon:playAnimation(root,id, wChairID)
    local Animation = nil
    if WSKGameCommon.tableConfig.wKindID == 51 or WSKGameCommon.tableConfig.wKindID == 55 or WSKGameCommon.tableConfig.wKindID == 56 or WSKGameCommon.tableConfig.wKindID == 57 or WSKGameCommon.tableConfig.wKindID == 58 or WSKGameCommon.tableConfig.wKindID == 59 then
        Animation = require("game.puke.AnimationNiu")
    elseif WSKGameCommon.tableConfig.wKindID == 53 then
    ...
    local AnimationData = Animation[id][WSKGameCommon.regionSound]
    ...
    if AnimationData.animFile ~= "" then
        if id == "报警" then
            ...
            if WSKGameCommon.tableConfig.wKindID == 51 or WSKGameCommon.tableConfig.wKindID == 53 or WSKGameCommon.tableConfig.wKindID == 55 or WSKGameCommon.tableConfig.wKindID == 56 or WSKGameCommon.tableConfig.wKindID == 57 or WSKGameCommon.tableConfig.wKindID == 58 or WSKGameCommon.tableConfig.wKindID == 59 then
    ...
    --     soundFile = AnimationData.sound[WSKGameCommon.player[wChairID].cbSex]
    -- else
    --     soundFile = AnimationData.sound[0]
    -- end
    -- if soundFile ~= "" then
    --     if id ~= "我先出" or wChairID == self:getRoleChairID()  then 
    --         require("common.Common"):playEffect(AnimationData.sound[WSKGameCommon.player[wChairID].cbSex])
    --     end  
    -- end
