...
local WSKPersonInfor	= class("WSKPersonInfor", cc.load("mvc").ViewBase)

function WSKPersonInfor:onConfig()
	self.widget			= {
		{'mask','onClose'},
		{'Image_avatar_player'},
		{'Text_name'},
...
function WSKPersonInfor:onEnter()
	
end

function WSKPersonInfor:onExit()
	cc.UserDefault:getInstance():setBoolForKey('PDKOpenUserEffect', self.isOpen)
end

function WSKPersonInfor:onCreate(param)
	local data = param[1]
...
function WSKPersonInfor:updateInfo( data )
	...
	if data.dwShamUserID ~= 0 and (WSKGameCommon.tableConfig.nTableType == TableType_GoldRoom or WSKGameCommon.tableConfig.nTableType == TableType_RedEnvelopeRoom) then
        self.Text_id:setString('ID:' .. data.dwShamUserID)
    end
end

function WSKPersonInfor:updateControl( ... )
...
function WSKPersonInfor:onControl()
    --是否开启
    self.isOpen = not self.isOpen
    self:updateControl()
end

function WSKPersonInfor:getPlayerInfoByUserID(dwUserID)
...
function WSKPersonInfor:updateScrollviewImage()	
...
function WSKPersonInfor:buttonCall(sender)
	local index = sender:getName()
	print('===>>>>>>',index)
	local targetChair = nil   
	for key,info in pairs(WSKGameCommon.player or {}) do
	...
	NetMgr:getGameInstance():sendMsgToSvr(NetMsgId.MDM_GF_GAME, NetMsgId.SUB_GF_USER_EFFECTS, "www", tonumber(index), WSKGameCommon:getRoleChairID(),targetChair)
	self:removeFromParent()
end

function WSKPersonInfor:addListener(btn, callback)
...
function WSKPersonInfor:onClose()
	self:removeFromParent()
end
return WSKPersonInfor 