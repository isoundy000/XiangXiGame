...
function GameEndLayer:onCreate(pBuffer)       
    ...
    local Gold = math.floor( (pBuffer.lGameScore[WSKGameCommon.meChairID+1]*0.9)/ 1)
    local Money = math.floor( (pBuffer.lGameScore[WSKGameCommon.meChairID+1]*0.35)/ 1)
    ...
    Common:addTouchEventListener(uiButton_Gold,function() 
        WSKGameCommon:GetReward(0)
    end)
    Common:addTouchEventListener(uiButton_Money,function() 
        WSKGameCommon:GetReward(1)
    end)
    if WSKGameCommon.tableConfig.nTableType == TableType_RedEnvelopeRoom and pBuffer.lGameScore[WSKGameCommon.meChairID+1] > 0 then 
...
--    local uiPanel_look = ccui.Helper:seekWidgetByName(self.root,"Panel_look")
    ...
    if WSKGameCommon.tableConfig.nTableType == TableType_GoldRoom or WSKGameCommon.tableConfig.nTableType == TableType_RedEnvelopeRoom then
        uiText_info:setString(string.format("倍率：%d\n本局消耗：%d",WSKGameCommon.tableConfig.wCellScore,WSKGameCommon.tableConfig.wCellScore*0.5))
    end
    local uiImage_result = ccui.Helper:seekWidgetByName(self.root,"Image_result")
    local viewID = WSKGameCommon:getViewIDByChairID(pBuffer.wWinUser)
    ...
    for key, var in pairs(WSKGameCommon.player) do
        local viewID = WSKGameCommon:getViewIDByChairID(var.wChairID)           
        ...
        if pBuffer.bUserCardCount[key+1] >= 17 then--WSKGameCommon.gameConfig.bSpringMinCount
    ...
    for i = WSKGameCommon.gameConfig.bPlayerCount+1, 3 do
