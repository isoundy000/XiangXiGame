...
function WSKSetting:onCreate(params)
	self:initValue()
...
function WSKSetting:registerSliderEvent( ... )
--音乐
...
function WSKSetting:onMusicFunc()
...
function WSKSetting:onEffectFunc( ... )
...
function WSKSetting:changeSizeCallFunc(sender)
...
function WSKSetting:changeBgCallFunc( sender )
	local name = sender:getName()
...
function WSKSetting:changeBtnVoiceCallFunc( sender )
	self.isShowVoice = not self.isShowVoice
	self:updateVoice()
end

function WSKSetting:getChangeValue( value )
	if value == 1 then
		value = 2
	else
		value = 1 
...
function WSKSetting:updateSetting( )
	self:updateSlider()
	self:updateHDDH(self.bgNum,'button_bg_%d',1,4)
	self:updateHDDH(self.size,'button_card_bg_%d',1,2)
	self:updateVoice()
...
function WSKSetting:updateVoice( ... )
...
function WSKSetting:updateSlider( ... )
...
function WSKSetting:onClose(...)
    self:saveSetting()
	self:removeFromParent()
	EventMgr:dispatch(EventType.EVENT_TYPE_SKIN_CHANGE)
end

function WSKSetting:saveSetting( ... )
...
function WSKSetting:getDefaultValue( key,default )
    return cc.UserDefault:getInstance():getIntegerForKey(key,default)
end

function WSKSetting:getVoice( min,max,cur )
	if cur >= max then
		cur = max
	elseif cur <= min then
		cur = min
...
function WSKSetting:onDimissCallFunc(  )
...
--添加slider event
function WSKSetting:addSliderEvent(slider,callBack)
	if slider then
