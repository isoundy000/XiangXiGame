...
--   聊天
---------------
local WSKChat  = class("WSKChat ", cc.load("mvc").ViewBase)
...
function WSKChat :onCreate(params)
...
function WSKChat :initEmSetting(...)
	--50 为了做分页给每页表情赋值一个id
	self:initOneEmjio('page1_Button_1', 'page1_ScrollView_1', 'press_1', 23, 0)
end

--初始化表情 --page1_ScrollView_1
function WSKChat :initOneEmjio(btnName, listName, press, index, start)
...
function WSKChat :addMaskListen(...)
...
function WSKChat :addPanelListen(item, call)
...
function WSKChat :closeView(...)
	self:setVisible(false)
end

function WSKChat :onSendCall(...)
	local contents = ''
	if self.isOS then
...
function WSKChat :clickExpressLab(sender)
...
function WSKChat :onClickHead(sender)
	local name = sender:getName()
	
	self:showPage(name)
end

function WSKChat :showPage( hedeName )
...
function WSKChat :buttonCall(sender)
...
function WSKChat :emButtonCall(sender)
	local name = sender:getName()
	self:showPage(name)
end


function WSKChat :addListener(btn, callback)
...
function WSKChat :SUB_GR_SEND_CHAT(event)

end

function WSKChat :SUB_GF_USER_EXPRESSION(event)
