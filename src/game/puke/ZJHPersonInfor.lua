...
local ZJHPersonInfor	= class("ZJHPersonInfor", cc.load("mvc").ViewBase)

function ZJHPersonInfor:onConfig()
	self.widget			= {
		{'mask','onClose'},
		{'Image_avatar_player'},
		{'Text_name'},
...
function ZJHPersonInfor:onEnter()
	
end

function ZJHPersonInfor:onExit()
	cc.UserDefault:getInstance():setBoolForKey('PDKOpenUserEffect', self.isOpen)
end

function ZJHPersonInfor:onCreate(param)
	local data = param[1]
...
function ZJHPersonInfor:updateInfo(data)
	...
	if data.dwShamUserID ~= 0 and (ZJHGameCommon.tableConfig.nTableType == TableType_GoldRoom or ZJHGameCommon.tableConfig.nTableType == TableType_RedEnvelopeRoom) then
        self.Text_id:setString('ID:' .. data.dwShamUserID)
    end
end

function ZJHPersonInfor:updateControl( ... )
...
function ZJHPersonInfor:onControl()
    --是否开启
    self.isOpen = not self.isOpen
    self:updateControl()
end

function ZJHPersonInfor:getPlayerInfoByUserID(dwUserID)
...
function ZJHPersonInfor:updateScrollviewImage()	
...
function ZJHPersonInfor:buttonCall(sender)
	local index = sender:getName()
	print('===>>>>>>',index)
	local targetChair = nil   
	for key,info in pairs(ZJHGameCommon.player or {}) do
	...
	local playerNum = 0
	for k, v in pairs(ZJHGameCommon.player or {}) do
	...
	NetMgr:getGameInstance():sendMsgToSvr(NetMsgId.MDM_GF_GAME, NetMsgId.SUB_GF_USER_EFFECTS, "www", tonumber(index), ZJHGameCommon:getRoleChairID(),targetChair)
	self:removeFromParent()
end

function ZJHPersonInfor:addListener(btn, callback)
...
function ZJHPersonInfor:onClose()
	self:removeFromParent()
end
return ZJHPersonInfor 