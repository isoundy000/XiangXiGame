...
function ZJHGameCommon:init()
    self.gameState = 0
end

function ZJHGameCommon:getViewIDByChairID(wChairID)
...
function ZJHGameCommon:getRoleChairID()
    return self.meChairID
end

function ZJHGameCommon:ContinueGame(cbLevel)
    NetMgr:getGameInstance():sendMsgToSvr(NetMsgId.MDM_GR_USER,NetMsgId.REQ_GR_USER_SET_POSITION,"aad",LocationSystem.pos.x, LocationSystem.pos.y, ZJHGameCommon.dwUserID)
    if ZJHGameCommon.tableConfig.nTableType == TableType_FriendRoom or ZJHGameCommon.tableConfig.nTableType == TableType_ClubRoom then
...
function ZJHGameCommon:getUserInfoByUserID(dwUserID)
    for key, var in pairs(self.player) do
...
function ZJHGameCommon:rad(d)
    return d* math.pi / 180.0;
end 

function ZJHGameCommon:GetDistance(lat1,lat2)
...
--牌资源
function ZJHGameCommon:getCardNode(data)
    ...
    card = ccui.ImageView:create(string.format("puke/card/card0/puke_%d_%d.png",color,value))
        ...
        if ZJHGameCommon.gameConfig.bFKSLaiZi == 1 and  value == 3 and color == 3  then 
...
function ZJHGameCommon:playAnimation(root,id, wChairID)
    local Animation = require("game.puke.AnimationHSP")
    if Animation[id] == nil then
        return
    end
    local AnimationData = Animation[id][ZJHGameCommon.regionSound]
    if AnimationData == nil then
        return
    end
    local soundFile = ""
    if wChairID ~= nil then
        soundFile = AnimationData.sound[ZJHGameCommon.player[wChairID].cbSex]
    else
        soundFile = AnimationData.sound[0]
    end
    if soundFile ~= "" then
        require("common.Common"):playEffect(AnimationData.sound[ZJHGameCommon.player[wChairID].cbSex])
    end
